...
EXCEPTION_CLASS(control_connection_timeout_exception, exception)
EXCEPTION_CLASS(control_connection_json_exception, exception)

namespace adns
{
    /**
     * Handle a single tcp control connection. Reads http requests messages off the 
     * wire, handles them, then sends back the results.
     */
    class control_connection final
    {
    public:
         ...
         */
        control_connection(server_config &config, thread_pool<control_connection> &pool);

        /**
         * kill threads etc.
         */
        virtual ~control_connection();
    ...
    private:
        ...
        thread_pool<control_connection> &m_pool;
